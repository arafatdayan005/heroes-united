...
function ManageUsers() {
    ...
    useEffect(() => {
        fetch(`https://heroes-united-server-arafatdayan005.vercel.app/users`)
            .then(res => res.json())
    ...
    const blockUser = (id) => {
        ...
        fetch(`https://heroes-united-server-arafatdayan005.vercel.app/changeRole/${id}`, {
            method: 'PUT',
            headers: {
                        ...
                        <tbody>
                            {
                                data.map((d, i) =>
                                    <tr key={d._id} className="bg-white text-center border-b dark:bg-gray-700 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td className="py-4">
                                            <button onClick={() => blockUser(d._id)} type="button" className={`text-white bg-red-500 ${d.role === "admin" ? "cursor-not-allowed" : "hover:bg-red-600"}  focus:outline-none font-medium rounded-full text-sm px-5 py-2.5 text-center`} disabled={d.role === "admin"}>Block</button>
                                        </td>
                                    </tr>
                                )
                            }
